...
@Component({
  selector: 'app-servicemoveinternational',
  templateUrl: './servicemoveinternational.page.html',
  styleUrls: ['./servicemoveinternational.page.scss'],
})
export class ServicemoveinternationalPage implements OnInit {
  ...
  formvalue={
    ...
    booking_type:"Moving Internationally",
    additional_services:{handyman:false,storage:false},
  	images:[],
...
updateinfo(){
    ...
    if(this.formvalue.additional_services.handyman==true){
    	var handyman_services="Yes";
    }else{
    	var handyman_services="No";
    }

    if(this.formvalue.additional_services.storage==true){
    	var storage_services="Yes";
    }else{
    	var storage_services="No";
    }
    ...
    this.formvalue.booking_details=[
      ...
      {
        'title':'Need Handyman Services?',
        'title_ar':'تحتاج خدمات العامل الماهر؟',
        'value':handyman_services,
      },
      {
        'title':'Need Storage Service?',
        'title_ar':'تحتاج خدمات التخزين؟',
        'value':storage_services,
      },
      {
        'title':'Moving From',
        'title_ar':'الانتقال من',
        'value':this.formvalue.start_country+" "+this.formvalue.start_address,
      },

      {
        'title':'Moving to',
        'title_ar':'الانتقال إلى',
        'value':this.formvalue.destination_country+" "+this.formvalue.destination_address
      },
...
if(this.formvalue.start_country==""){
          this.diginix.toast("Start country is incomplete.",500);
        return false;
}


if(this.formvalue.start_address==""){
          this.diginix.toast("Start address is incomplete.",500);
        return false;
}



if(this.formvalue.destination_country==""){
          this.diginix.toast("Destination country is incomplete.",500);
        return false;
}


if(this.formvalue.destination_address==""){
          this.diginix.toast("Destination address is incomplete.",500);
        return false;
}
